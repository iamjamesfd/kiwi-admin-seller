<template>
  <div class="relative" :class="$attrs.tabBarClass">
    <div
      class="flex overflow-auto border-b border-b-stroke-primary"
      @scroll="changeTab(activeTab)"
    >
      <div
        :key="index"
      ...
      >
        <slot name="tabButton" :tab="{ tab, isActive: activeTab === index }">
          <button
            class="medium-bold w-full shrink-0 px-3 py-4 text-sm text-font-accent-1 transition-colors"
            :class="{
...
const props = withDefaults(
  defineProps<{ tabs?: string[]; mode?: "full" | "inline" }>(),
  { mode: "inline" },
);
